unit class PDF::To::Cairo:ver<0.0.8>;
...
method (*@) is also<
    SetWordSpacing SetCharSpacing
    SetStrokeRGB  SetStrokeCMYK SetStrokeGray SetStrokeColorSpace SetStrokeColor  SetStrokeColorN
    SetFillRGB    SetFillCMYK   SetFillGray   SetFillColorSpace   SetFillColor    SetFillColorN
> { }

# - These methods update the text state for later reference
method ($) is also<SetTextLeading SetTextRise SetHorizScaling SetTextRender> { }
# - These methods update text state and also reset text-flow
method (*@) is also<BeginText SetTextMatrix TextMove TextNextLine TextMoveSet> {
}
# - These methods have no affect on rendering
method (*@) is also<BeginMarkedContent BeginMarkedContentDict EndMarkedContent MarkPointDict BeginExtended EndExtended> { }
